            ...
            if (data) {
                zonas.forEach(function (zone) {
                    ...
                    zone.sensors.forEach(function (sensor) {
                        $('#zone-' + zone.id)
                            .append('<div> <span><b> Sensor ' + sensor.id + '</b><br> Temperatura mínima: </span> <input id="temp-range-' + sensor.id + '" type="range" min="0" max="50" step="10"><input id="temp-number-' + sensor.id + '"type="number" style="border:none; width: 3rem; margin-left: 1rem;" disabled> <br>' +  '<span>' + ' Humedad mínima: </span> <input id="hume-range-' + sensor.id + '" type="range" min="0" max="100" step="10"><input id="hume-number-' + sensor.id + '"type="number" style=" border:none; width: 3rem; margin-left: 1rem;" disabled>' + '<button style="margin-left:1rem;" class="btn btn-outline-primary col-2 pl-2" onclick="save_sensor(' + sensor.id + ')">' + 'Save</button>' + '</div>');
